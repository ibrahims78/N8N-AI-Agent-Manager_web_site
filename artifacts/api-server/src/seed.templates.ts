 ...
 * seed.templates.ts
 *
 * Phase 5B (transitional export): the canonical SYSTEM_TEMPLATES live here
 * temporarily so that the one-shot extraction script
 * (`scripts/extract-templates.mjs`) can write them out to JSON files under
 * `lib/n8n-nodes-catalog/templates/`. Once the JSON files are committed,
 * `seed.ts` reads from disk via `loadSystemTemplates()` and this exported
 * constant remains as a self-check fallback (never imported at runtime in
 * the bundled server).
 */
...
export const SYSTEM_TEMPLATES_SOURCE: SystemTemplate[] = [
  {
    slug: "automated-email-sender",
    name: "إرسال إيميل تلقائي",
    nameEn: "Automated Email Sender",
    description:
      "إرسال إشعارات بريد إلكتروني تلقائية عند حدوث أحداث محددة عبر Gmail",
    descriptionEn:
      "Send automated email notifications via Gmail when specific events occur",
    category: "email",
    nodesCount: 4,
    avgRating: 4.5,
    workflowJson: {
  ...
  {
    slug: "webhook-data-receiver",
    name: "استقبال Webhook ومعالجة البيانات",
    nameEn: "Webhook Data Receiver & Processor",
    description:
      "استقبال البيانات عبر Webhook، التحقق منها، ومعالجتها وحفظها في قاعدة بيانات",
    descriptionEn:
      "Receive data via Webhook, validate, process and store it in a database",
    category: "api",
    nodesCount: 5,
    avgRating: 4.2,
    workflowJson: {
      ...
      nodes: [
        ...
        },
        {
          ...
          parameters: {
            ...
            respondWith: "json",
            responseBody:
              "={{ JSON.stringify({ success: true, id: $json.id, message: 'تم استلام البيانات بنجاح' }) }}",
          },
        },
        {
          id: "respond-err",
          ...
          parameters: {
            ...
            respondWith: "json",
            responseBody:
              '{"success":false,"message":"البيانات غير صالحة"}',
          },
        },
      ],
      connections: {
  ...
  {
    slug: "automated-daily-report",
    name: "تقرير يومي تلقائي",
    nameEn: "Automated Daily Report",
    description:
      "جلب البيانات من مصادر متعددة وإنشاء وإرسال تقرير يومي شامل في وقت محدد",
    descriptionEn:
      "Fetch data from multiple sources and generate & send a comprehensive daily report at a scheduled time",
    category: "reports",
    nodesCount: 6,
    avgRating: 4.7,
    workflowJson: {
  ...
  },
  {
    ...
    description:
      "جلب بيانات من API خارجي ومزامنتها تلقائياً مع Google Sheets كل ساعة",
    descriptionEn:
      "Fetch data from an external API and automatically sync it with Google Sheets every hour",
    category: "reports",
    nodesCount: 4,
    avgRating: 4.0,
    workflowJson: {
    ...
    workflowJson: {
      ...
      nodes: [
        ...
        },
        {
          ...
          parameters: {
            jsCode:
              "return [{ json: { ...($input.item.json), executed: true, executedAt: new Date().toISOString(), result: 'success' } }];",
          },
        },
        {
          id: "code-6",
      ...
      connections: {
        "Every Morning": {
          main: [[{ node: "Define Tasks", type: "main", index: 0 }]],
        },
        "Define Tasks": {
  ...
  {
    slug: "periodic-database-cleanup",
    ...
    descriptionEn:
      "Automatically delete old records and optimize the database every week",
    category: "database",
    nodesCount: 5,
    avgRating: 4.1,
    workflowJson: {
      ...
      nodes: [
        ...
        },
        {
          ...
          parameters: {
            ...
            sendQuery: true,
            queryParameters: {
              parameters: [{ name: "before", value: "={{ $json.cutoffDate }}" }],
            },
            options: {},
          },
        },
      ...
      connections: {
        ...
        "Calculate Cutoff Date": {
          main: [[{ node: "Call Cleanup API", type: "main", index: 0 }]],
        },
        "Call Cleanup API": {
