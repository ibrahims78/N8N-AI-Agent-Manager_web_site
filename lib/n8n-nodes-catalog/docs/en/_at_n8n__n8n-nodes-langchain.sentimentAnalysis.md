...
`` if the input is from a chat or message source. By default, it expects a `text` field.
...
2. Set the "Text to Analyze" field to the relevant item property (for example, `` for blog post content).
