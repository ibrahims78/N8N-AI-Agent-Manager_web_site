# عقدة Discourse

استخدم عقدة Discourse لأتمتة سير العمل في Discourse، ودمج Discourse مع تطبيقات أخرى. تدعم n8n مجموعة واسعة من ميزات Discourse بشكل مدمج، بما في ذلك إنشاء الفئات والمجموعات والمنشورات والمستخدمين واستردادها وتحديثها وإزالتها.

في هذه الصفحة، ستجد قائمة بالعمليات التي تدعمها عقدة Discourse وروابط لموارد إضافية.

> **بيانات الاعتماد**
>
> ارجع إلى [بيانات اعتماد Discourse](/integrations/builtin/credentials/discourse.md) للحصول على إرشادات حول إعداد المصادقة.

## العمليات

*   الفئة
    *   إنشاء فئة
    *   استرداد جميع الفئات
    *   تحديث فئة
*   المجموعة
    *   إنشاء مجموعة
    *   استرداد مجموعة
    *   استرداد جميع المجموعات
    *   تحديث مجموعة
*   المنشور
    *   إنشاء منشور
    *   استرداد منشور
    *   استرداد جميع المنشورات
    *   تحديث منشور
*   المستخدم
    *   إنشاء مستخدم
    *   استرداد مستخدم
    *   استرداد جميع المستخدمين
*   مجموعة المستخدمين
    *   إضافة مستخدم إلى مجموعة
    *   إزالة مستخدم من مجموعة

## القوالب والأمثلة

<!-- see https://www.notion.so/n8n/Pull-in-templates-for-the-integrations-pages-37c716837b804d30a33b47475f6e3780 -->

## ماذا تفعل إذا كانت عمليتك غير مدعومة

إذا كانت هذه العقدة لا تدعم العملية التي ترغب في تنفيذها، يمكنك استخدام [عقدة HTTP Request](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/index.md) لاستدعاء واجهة برمجة تطبيقات الخدمة.

يمكنك استخدام بيانات الاعتماد التي أنشأتها لهذه الخدمة في عقدة HTTP Request:

1.  في عقدة HTTP Request، حدد **المصادقة** > **نوع بيانات الاعتماد المُحددة مسبقًا**.
2.  حدد الخدمة التي ترغب في الاتصال بها.
3.  حدد بيانات الاعتماد الخاصة بك.

ارجع إلى [عمليات API المخصصة](/integrations/custom-operations.md) لمزيد من المعلومات.