...
توفر n8n عقدة مُحفِّز (Trigger) لـ Airtable. يمكنك العثور على وثائق عقدة المُحفِّز [هنا](/integrations/builtin/trigger-nodes/n8n-nodes-base.airtabletrigger.md).
...
لجلب البيانات لسجل معين، تحتاج إلى معرّف السجل. هناك طريقتان للحصول على معرّف السجل.

### إنشاء عمود معرّف السجل (Record ID) في Airtable

لإنشاء عمود `Record ID` في جدولك، ارجع إلى هذه [المقالة](https://support.airtable.com/docs/finding-airtable-ids). يمكنك بعد ذلك استخدام معرّف السجل هذا في عقدة Airtable الخاصة بك.

### استخدام عملية السرد (List)

للحصول على معرّف السجل الخاص بسجلك، يمكنك استخدام عملية **السرد** لعقدة Airtable. ستُرجع هذه العملية معرّف السجل بالإضافة إلى الحقول. يمكنك بعد ذلك استخدام معرّف السجل هذا في عقدة Airtable الخاصة بك.

### تصفية السجلات عند استخدام عملية القائمة
...
2.  أدخل معرف القاعدة (Base ID) واسم الجدول (Table name) في حقلي **معرف القاعدة** و**الجدول**، على التوالي.
3.  انقر على **إضافة خيار** (Add Option) وحدد 'Filter By Formula' من القائمة المنسدلة.
4.  أدخل الصيغة التالية في حقل **التصفية بواسطة صيغة**: `{Organization}='n8n'`.
...
للاطلاع على الأخطاء أو المشكلات الشائعة وخطوات الحل المقترحة، ارجع إلى [المشكلات الشائعة](/integrations/builtin/app-nodes/n8n-nodes-base.airtable/common-issues.md).

---

# المشكلات الشائعة لعقدة Airtable

فيما يلي بعض الأخطاء والمشكلات الشائعة المتعلقة بـ [عقدة Airtable](/integrations/builtin/app-nodes/n8n-nodes-base.airtable/index.md) وخطوات حلها أو استكشافها.

## محظور - ربما تحقق من بيانات الاعتماد الخاصة بك

يظهر هذا الخطأ عند محاولة تنفيذ إجراءات غير مسموح بها بمستوى الوصول الحالي الخاص بك. النص الكامل يبدو كالتالي:

```
There was a problem loading the parameter options from server: "Forbidden - perhaps check your credentials?"
```

يظهر الخطأ غالبًا عندما لا تحتوي بيانات الاعتماد التي تستخدمها على النطاقات (scopes) المطلوبة للموارد التي تحاول إدارتها.

ارجع إلى [بيانات اعتماد Airtable](/integrations/builtin/credentials/airtable.md) و[توثيق نطاقات Airtable](https://airtable.com/developers/web/api/scopes) للمزيد من المعلومات.

## الخدمة تتلقى عددًا كبيرًا جدًا من الطلبات منك

تفرض Airtable حدًا صارمًا على واجهة برمجة التطبيقات (API) لعدد الطلبات التي يتم إنشاؤها باستخدام رموز الوصول الشخصية (personal access tokens).

إذا أرسلت أكثر من خمسة طلبات في الثانية لكل قاعدة، ستتلقى خطأ 429، مما يشير إلى أنك أرسلت عددًا كبيرًا جدًا من الطلبات. سيتعين عليك الانتظار 30 ثانية قبل استئناف الطلبات. ينطبق هذا الحد نفسه على إرسال أكثر من 50 طلبًا عبر جميع القواعد لكل رمز وصول (access token).
